...
def create_voting_dict(strlist):
    ...
    """
    return {i.split()[0]:list(map(int, i.split()[3:])) for i in strlist}
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    return sum(v1 * v2 for v1,v2 in zip(voting_dict[sen_a], voting_dict[sen_b]))
...
def most_similar(sen, voting_dict):
    ...
    similarity = {s:policy_compare(sen, s, voting_dict) for s in voting_dict.keys() if s != sen }
