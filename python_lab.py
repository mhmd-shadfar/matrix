...
# {x*y for x in X2 for y in Y2} evaluates to a five-element set.
...
# other way to calculate intersection
...
odd_num_list_range = {range(1, 100, 2)}
...
k = 'Frodo'
value_list_modified_2 = [x[k] for x in dlist if k in x] # <-- as you do here
...
representation_dict = { d:[(d // base // base)%base,(d // base)%base, d%base] for d in range(1000) }
...
def all_3_digit_numbers(base, digits): return { i for i in range(base**3) if i <=999}
